...
#' is obtained from the overlap.matrix parameter.
#'
#' @param all.regions A GRanges object with the regions represented
#    by the rows of overlap.matrix.
#' @param overlap.matrix A matrix, with rows corresponding to regions
#'   in all.rgions, and columns corresponding to proteins/factors. A
...
#' 
#' Given an intersect.object , finds all regions where all
#' factors described by either indices or names are present. 
#' If both indices and names are NULL, the inner intersection
#' is calculated.
#'
#' @param intersect.object An intersect.object returned by build.intersect.
#'   by the rows of overlap.matrix.
#' @param indices A vector of indices into the factors of intersect.object.
...
#'
#' @param grl The GRangesList object whose elements need to be overlapped with
#'   each others.
...
#'
#' @param intersect.object An intersect object returned by build.intersect.
#' @param filename A filename for the resulting venn diagram. Pass NULL
...
#'
#' Given an intersect.object, generate annotations for the regions
...
#    intersect.object. The row's factor is used as a denominator. Therefore, the
